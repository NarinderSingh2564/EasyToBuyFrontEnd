...
export class CustomerOrdersComponent {
  ...
  getOrderList() {
    this.orderService.getOrderList(this.accountService.getCustomerId(),0, "", 0, "", "").subscribe((result: any) => {
      this.orderList = result
    })
  }

  searchProducts(searchText: string) {
    this.orderService.getOrderList(this.accountService.getCustomerId(), 0, searchText, 0, "", "").subscribe((result: any) => {
      this.orderList = result
    })
  }

  filterOrdersByType(event: any) {
    this.orderService.getOrderList(this.accountService.getCustomerId(), 0, "", event.target.value, "", "").subscribe((result: any) => {
      this.orderList = result
    })
  }
  ...
  filterOrdersByMonth(event: any) {
    if (event.target.value == 1) {
      ...
      this.orderService.getOrderList(this.accountService.getCustomerId(), 0, "", 0, FirstDate.toLocaleDateString(), (new Date()).toLocaleDateString()).subscribe((result: any) => {
        this.orderList = result
      })
    }
  ...
  filterOrdersByDates(firstDate: any, secondDate: any) {
    this.orderService.getOrderList(this.accountService.getCustomerId(), 0, "", 0, firstDate, secondDate).subscribe((result: any) => {
      this.orderList = result
    })
  }
